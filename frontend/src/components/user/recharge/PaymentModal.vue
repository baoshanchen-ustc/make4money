...
const handleStatusChange = (status: string) => {
  if (status === 'paid') {
    ...
    router.push({ name: 'RechargeSuccess', params: { orderNo: props.orderNo } })
...
const pollOrderStatus = async () => {
  try {
    ...
    console.error('[PaymentModal] Poll failed:', error)
  }
}
  ...
  try {
    ...
    if (result.status === 'pending') {
      ...
      if (!result.qrcode_url && !result.prepay_id) {
        const payResult = await rechargeAPI.initiatePayment(props.orderNo)
    ...
    console.error('[PaymentModal] Load order failed:', error)
  } finally {
    loading.value = false
  }
}
