              ...
              <div class="card p-5">
                ...
                <div class="mb-3 flex flex-wrap items-center gap-2">
                  <span :class="['rounded-md border px-2 py-0.5 text-xs font-medium', planBadgeClass]">
                    {{ selectedPlan.group_display_name || platformLabel(selectedPlan.group_platform || '') }}
                  </span>
              ...
              <div v-if="activeSubscriptions.length > 0">
                ...
                <div class="space-y-2">
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    ...
                    <div :class="['h-6 w-1 shrink-0 rounded-full', platformAccentBarClass(sub.group?.platform || '', sub.group?.display_icon)]" />
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-1.5">
                        <span class="truncate text-xs font-semibold text-gray-900 dark:text-white">{{ sub.group?.display_name || sub.group?.name || t('payment.groupFallback', { id: sub.group_id }) }}</span>
                        <span :class="['shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-medium', platformBadgeLightClass(sub.group?.platform || '', sub.group?.display_icon)]">{{ sub.group?.display_name || platformLabel(sub.group?.platform || '') }}</span>
                      </div>
...
// 优先用 group_display_icon 决定配色，避免暴露真实 platform（如 deepseek）
const planBadgeClass = computed(() => platformBadgeClass(selectedPlan.value?.group_platform || '', selectedPlan.value?.group_display_icon))
const planTextClass = computed(() => platformTextClass(selectedPlan.value?.group_platform || '', selectedPlan.value?.group_display_icon))
