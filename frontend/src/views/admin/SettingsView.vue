...
type ProviderDefaultSettingsSource = Exclude<DefaultSettingsSource, "email">;
...
const defaultSettingsBySource = reactive<
  Record<DefaultSettingsSource, ProviderDefaultUserSettings>
>({
...
const defaultSettingsOverrideBySource = reactive<
  Record<ProviderDefaultSettingsSource, boolean>
>({
  linuxdo: false,
  wechat: false,
  oidc: false,
...
const form = reactive<SettingsForm>({
  ...
  wechat_login_open_app_secret_configured: false,
...
function syncLegacyDefaultSettingsFromEmail() {
  ...
  form.default_subscriptions = defaultSettingsBySource.email.subscriptions.map(
    (item) => ({
      group_id: item.group_id,
      validity_days: item.validity_days,
    }),
...
function buildInheritedProviderDefaultSettings(): ProviderDefaultUserSettings {
  return createProviderDefaultSettings({
    apply_on_bind: false,
    balance: defaultSettingsBySource.email.balance,
    concurrency: defaultSettingsBySource.email.concurrency,
    subscriptions: defaultSettingsBySource.email.subscriptions.map((item) => ({
      group_id: item.group_id,
      validity_days: item.validity_days,
    })),
  });
...
function syncInheritedProviderDefaultsFromEmail() {
  (["linuxdo", "wechat", "oidc"] as ProviderDefaultSettingsSource[]).forEach(
    (source) => {
      if (!defaultSettingsOverrideBySource[source]) {
        Object.assign(
          defaultSettingsBySource[source],
          buildInheritedProviderDefaultSettings(),
        );
      }
    },
  );
...
function setProviderDefaultOverride(
  source: DefaultSettingsSource,
  enabled: boolean,
) {
  if (source === "email") return;
  defaultSettingsOverrideBySource[source] = enabled;
  if (!enabled) {
    Object.assign(
      defaultSettingsBySource[source],
      buildInheritedProviderDefaultSettings(),
...
function setProviderDefaultSettings(
  ...
  Object.assign(
    defaultSettingsBySource[source],
    createProviderDefaultSettings(settings ?? {}),
  );
}
...
async function loadSettings() {
  ...
  try {
    ...
    setProviderDefaultSettings(
      "email",
      settings.default_settings_email ?? {
    ...
    defaultSettingsOverrideBySource.linuxdo =
      settings.default_settings_linuxdo_overridden === true;
    defaultSettingsOverrideBySource.wechat =
      settings.default_settings_wechat_overridden === true;
    defaultSettingsOverrideBySource.oidc =
      settings.default_settings_oidc_overridden === true;
    syncLegacyDefaultSettingsFromEmail();
    syncInheritedProviderDefaultsFromEmail();
    registrationEmailSuffixWhitelistTags.value =
      normalizeRegistrationEmailSuffixDomains(
...
function removeDefaultSubscription(
  source: DefaultSettingsSource,
  index: number,
) {
...
async function saveSettings() {
  ...
  try {
    ...
    const normalizeProviderDefaultSettingsForSave = (
      ...
      const duplicateDefaultSubscription = normalizedSubscriptions.find(
        (item) => {
    ...
    const normalizedEmailDefaultSettings =
      normalizeProviderDefaultSettingsForSave("email");
    const normalizedLinuxDoDefaultSettings =
      normalizeProviderDefaultSettingsForSave("linuxdo");
    const normalizedWeChatDefaultSettings =
      normalizeProviderDefaultSettingsForSave("wechat");
    const normalizedOIDCDefaultSettings =
      normalizeProviderDefaultSettingsForSave("oidc");
    if (
    ...
    Object.assign(
      defaultSettingsBySource.email,
      normalizedEmailDefaultSettings,
    );
    syncInheritedProviderDefaultsFromEmail();
    const inheritedProviderDefaultSettings =
      buildInheritedProviderDefaultSettings();
    Object.assign(
      defaultSettingsBySource.linuxdo,
      defaultSettingsOverrideBySource.linuxdo
        ? normalizedLinuxDoDefaultSettings
        : inheritedProviderDefaultSettings,
    );
    Object.assign(
      defaultSettingsBySource.wechat,
      defaultSettingsOverrideBySource.wechat
        ? normalizedWeChatDefaultSettings
        : inheritedProviderDefaultSettings,
    );
    Object.assign(
      defaultSettingsBySource.oidc,
      defaultSettingsOverrideBySource.oidc
        ? normalizedOIDCDefaultSettings
        : inheritedProviderDefaultSettings,
    );
    ...
    const payload: UpdateSettingsRequest = {
      ...
      third_party_first_login_require_email:
        form.third_party_first_login_require_email,
      registration_email_suffix_whitelist:
      ...
      default_settings_linuxdo: defaultSettingsOverrideBySource.linuxdo
        ? normalizedLinuxDoDefaultSettings
        : inheritedProviderDefaultSettings,
      default_settings_linuxdo_overridden:
        defaultSettingsOverrideBySource.linuxdo,
      default_settings_wechat: defaultSettingsOverrideBySource.wechat
        ? normalizedWeChatDefaultSettings
        : inheritedProviderDefaultSettings,
      default_settings_wechat_overridden:
        defaultSettingsOverrideBySource.wechat,
      default_settings_oidc: defaultSettingsOverrideBySource.oidc
        ? normalizedOIDCDefaultSettings
        : inheritedProviderDefaultSettings,
      default_settings_oidc_overridden: defaultSettingsOverrideBySource.oidc,
      ...
      wechat_login_open_app_secret:
        form.wechat_login_open_app_secret || undefined,
...
const enabledProviderKeyOptions = computed(() => {
  return providerKeyOptions.value.filter((opt) =>
    enabledProviderKeys.value.includes(opt.value as PaymentProviderKey),
  );
});
...
function openCreateProvider() {
  ...
  providerDialogRef.value?.reset(
    (enabledProviderKeyOptions.value[0]?.value ||
      "easypay") as PaymentProviderKey,
  );
