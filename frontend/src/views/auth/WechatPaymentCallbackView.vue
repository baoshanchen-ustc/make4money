    ...
    <div class="mx-auto max-w-2xl">
      <div class="card p-6">
        <h1 class="text-lg font-semibold text-gray-900 dark:text-white">
          {{ t('auth.wechatPayment.callbackTitle') }}
        </h1>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
          {{ errorMessage || t('auth.wechatPayment.callbackProcessing') }}
        </p>

        <div v-if="!errorMessage" class="mt-6 flex items-center justify-center py-10">
        ...
        >
          ...
          <button class="btn btn-primary mt-4" type="button" @click="goBackToPayment">
...
onMounted(async () => {
  ...
  if (!openid) {
    errorMessage.value = t('auth.wechatPayment.callbackMissingOpenId')
    return
  }
