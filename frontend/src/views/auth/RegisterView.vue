<template>
  <AuthLayout>
    <div class="space-y-6">
      ...
      <div
        v-if="linuxdoOAuthEnabled || oidcOAuthEnabled || wechatOAuthVisible"
        class="space-y-4"
      >
        <WechatOAuthSection
      ...
      <form v-else @submit.prevent="handleRegister" class="space-y-5">
        <!-- Email Input -->
        <div v-if="requiresLocalCredentials">
        ...
        <div v-if="invitationCodeEnabled">
          ...
          <transition name="fade">
            ...
            <p
              v-else-if="invitationValidation.invalid"
...
const pendingAuthRequiresEmailBinding = ref<boolean>(true);
const requiresLocalCredentials = computed<boolean>(
  () => !pendingAuthSession.value || pendingAuthRequiresEmailBinding.value,
);
const activeRedirectPath = computed(() =>
  sanitizeAuthRedirectPath(
    pendingAuthSession.value?.redirect ||
...
onMounted(async () => {
  try {
    ...
    pendingAuthRequiresEmailBinding.value =
...
async function validateInvitationCodeDebounced(code: string): Promise<void> {
  ...
  try {
    ...
    if (result.valid) {
      ...
      invitationValidation.message = getInvitationErrorMessage(
        result.error_code,
      );
    }
  } catch {
...
async function handleRegister(): Promise<void> {
  ...
  try {
    ...
    const adoptionDecision =
      getPendingAuthSessionAdoptionDecision(activePendingSession);

    // If email verification is enabled, redirect to verification page
    if (emailVerifyEnabled.value && requiresLocalCredentials.value) {
    ...
    if (activePendingSession) {
      const response = await createOAuthAccount(activePendingSession.provider, {
        ...
        email: requiresLocalCredentials.value ? formData.email : undefined,
        password: requiresLocalCredentials.value
          ? formData.password
          : undefined,
