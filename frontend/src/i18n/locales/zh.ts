export default {
  ...
  connectivityTest: {
    title: '连接测试',
    description: '通过当前网络/代理测试流式传输稳定性。发送一个长内容生成请求，检测代理是否存在60秒超时或连接中断问题。',
    testConfig: '测试配置',
    apiKey: 'API 密钥',
    ...
    metrics: '实时指标',
    elapsed: '已用时',
    tokensReceived: '收到 Token',
    chunksReceived: '收到块数',
    throughput: '速度',
    timeoutThreshold: '超时阈值（秒）',
    timeoutThresholdHint: '代理超时判断时间，默认 60 秒',
    streamDuration: '流式持续时长（秒）',
    streamDurationHint: '要求模型持续输出的时间，默认 70 秒',
    timeline: '时间线',
    events: '事件日志',
    noEvents: '等待开始...',
    streamPreview: '流式内容预览',
    streaming: '流式传输中',
    completed: '已完成',
    failed: '失败 / 已停止',
    testPassed: '测试通过',
    testFailed: '测试失败',
    diagnosis: '诊断',
    approachingTimeout: '⚠ 即将达到 {sec}s — 正在监测代理超时...',
    eventConnecting: '连接到 {endpoint}',
    eventModel: '模型：{model}',
    eventStreamMode: '模式：stream=true（Anthropic SSE）',
    eventHttpError: 'HTTP {status}：{body}',
    eventConnected: '已连接 — HTTP {status}',
    eventFirstChunk: '第一个数据块在 {ms}ms 时收到',
    eventMessageStop: '收到 message_stop 事件',
    eventActualModel: '实际模型：{model}',
    eventError: '上游错误：{msg}',
    eventStreamEnd: '流已结束（reader done）',
    eventUncleanEnd: '流在 {sec}s 时未正常终止就结束了',
    eventAborted: '用户停止了测试',
    eventFetchError: 'Fetch 错误：{msg}',
    eventSuccess: '在 {sec}s 内完成 — {tokens} 个 token',
    resultSuccess: '流在 {sec}s 内干净完成（{tokens} tokens，{chunks} 个块）',
    resultHttpError: 'HTTP 错误 {status} — 检查 API 密钥和接口地址',
    resultUnclean: '流在 {sec}s 时未正常终止',
    resultAborted: '用户在 {sec}s 后停止了测试',
    resultError: '错误：{msg}',
    diagnosisSlowButOk: '在 {sec}s 内成功完成。响应较慢但稳定，未检测到代理截断。',
    diagnosisGood: '优秀！流式传输快速且干净地完成。网络/代理状态健康。',
    diagnosisUnclean: '流结束时没有 [DONE] 或 message_stop，可能是传输途中连接被切断。',
    diagnosisTimeout: '连接在约 {sec}s 时断开。这是典型的代理/负载均衡器超时特征。您的代理可能设置了 {sec}s 的空闲或响应超时。建议换用没有流式超时的代理，或使用直连。',
    diagnosis60s: '连接在约60秒时断开。这是典型的代理/负载均衡器超时特征。您的代理可能设置了60秒的空闲或响应超时。建议换用没有流式超时的代理，或使用直连。',
    diagnosisQuickFail: '连接非常快就失败了。请检查 API 密钥、接口地址，以及服务是否可达。',
    diagnosisGeneral: '连接在 {sec}s 后断开，可能是代理超时、网络不稳定或上游限速。'
  },
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      copilot: {
        ...
        availableModels: '此 Key 可用的 Copilot 模型'
      }
    },
  ...
  admin: {
    ...
    accounts: {
      ...
      copilot: {
        ...
        planTypeHint: '选择你的 GitHub Copilot 订阅类型。个人版 Free/Pro/Pro+ 均使用同一 API 端点，商业版和企业版使用独立端点。',
    ...
    modelPricings: {
      title: '模型计费价格',
      description: '管理模型的 Fallback 计费价格（优先级：LiteLLM 动态价格 → 本页面配置 → 内置默认）',
      addEntry: '新增价格',
      editEntry: '编辑价格',
      deleteEntry: '删除',
      modelKey: '模型 Key',
      ...
      inputPrice: '输入价格',
      outputPrice: '输出价格',
      inputPricePriority: '输入价格（Priority）',
      outputPricePriority: '输出价格（Priority）',
      cacheReadPrice: '缓存读取价格',
      cacheReadPricePriority: '缓存读取（Priority）',
      priorityTipTitle: '什么是 Priority 价格？',
      priorityTipBody: 'OpenAI 提供两种服务层：标准层（Default）和优先层（Priority）。当请求携带 service_tier: "auto" 时，OpenAI 可能将请求路由至优先层，收取 Priority 单价（约 2×标准价）。若此处留空（填 0），系统将自动按标准价 × 2 估算。仅适用于支持双服务层的 OpenAI 模型，其他厂商模型无需填写。',
      priorityPlaceholder: '留空则按标准价 × 2',
      cacheCreationPrice: '缓存创建价格',
      perMillion: '/ 百万 Token',
      enabled: '启用',
      note: '备注',
      source: '价格来源',
      sourceLitellm: 'LiteLLM 动态',
      sourceDb: '数据库配置',
      sourceBuiltin: '内置默认',
      createSuccess: '价格配置创建成功',
      updateSuccess: '价格配置更新成功',
      deleteSuccess: '价格配置删除成功',
      confirmDelete: '确定要删除此价格配置吗？',
      noEntries: '暂无价格配置',
      calculationLogic: '计费逻辑说明',
      usdPerMillion: 'USD / 百万 Token',
      // 三层对比
    ...
    ops: {
      ...
      requestInspect: {
        ...
        kindAll: '全部',
        userFilter: '用户',
        userPlaceholder: '搜索用户邮箱...',
        anomalyFilter: '异常类型',
        anomalyPlaceholder: '全部',
        platform: '平台',
        searchQ: '关键词',
