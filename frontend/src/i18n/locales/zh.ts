export default {
  ...
  auth: {
    ...
    wechat: {
      callbackTitle: '正在从微信返回',
      callbackProcessing: '正在恢复支付上下文并返回结算页...',
      callbackMissingOpenId: '微信未返回可用的 OpenID，请重新发起支付。',
      backToPayment: '返回支付页'
    },
    oauth: {
      code: '授权码',
  ...
  admin: {
    ...
    settings: {
      ...
      wechat: {
        title: '微信 Connect 登录',
        description: '配置微信公众号网页授权；当前项目主要用于微信内支付获取 openid，也可复用于登录/注册入口。',
        enable: '启用微信登录',
        enableHint: '启用后可为登录/注册页准备微信入口，同时也为微信内支付提供公众号网页授权配置。',
        paymentUsageTitle: '这一组是“公众号网页授权”参数，不是微信支付商户参数',
        paymentUsageDesc: '当用户在微信内选择 `wxpay` / `wxpay_direct` 时，后端会先用这里的公众号 AppID / AppSecret 走网页授权拿 openid，再调用微信支付。',
        paymentCompatibilityHint: '如果这里没有配置：支付宝和站外支付不受影响，但微信内的 `wxpay` / `wxpay_direct` 会因拿不到 openid 而直接创建订单失败。',
        authDomainTitle: '微信公众平台里要配置的是“网页授权域名”',
        authDomainDesc: '微信侧应配置纯域名，例如 `crs.qazwc.com`。不要填完整 URL，不要带 `https://`，也不要带 `/api/v1/...` 路径。',
        authDomainDiff: '“网页授权域名”与“JS 安全域名”、“消息推送 URL / Token / EncodingAESKey”不是同一项；只配 JS 安全域名通常无法解决 `10003 redirect_uri 域名不一致`。',
        authDomainCallbackHint: '这里的“后端回调地址（Redirect URL）”填完整 URL，例如 `https://crs.qazwc.com/api/v1/auth/oauth/wechat/callback`；而微信公众平台侧只登记域名 `crs.qazwc.com`。',
        mpVerifyTitle: '域名校验文件注意事项',
        mpVerifyDesc: '如果微信公众平台要求上传 `MP_verify_xxx.txt` 校验文件，校验的是最终公网访问地址；这一步与这里填写的回调地址不是同一回事。',
        mpVerifyProxyHint: '站点前面如果还有 Nginx、Caddy、宝塔站点或 CDN，请把该文件放到最终对外站点根目录，或在反向代理层对 `/MP_verify_xxx.txt` 做精确返回，不能只放在应用容器里却仍被反代首页覆盖。',
        mpVerifyAccessHint: '完成后请确保 `https://你的域名/MP_verify_xxx.txt` 可以直接访问到纯文本文件内容；只有这样微信侧域名校验才会通过。',
        linkPublicPlatform: '打开微信公众平台',
        linkMerchantPlatform: '打开微信支付商户平台',
        linkWebAuthGuide: '查看公众号网页授权文档',
        linkJsapiGuide: '查看 JSAPI / Native 接入说明',
        publicSettingsHint:
          '管理员开关只负责配置后端设置。登录/注册页是否显示微信入口，还需要后端在公开设置中额外暴露 `wechat_oauth_enabled`。',
        appId: 'App ID',
        appIdPlaceholder: 'wx1234567890abcdef',
        appIdHint: '当前项目微信内支付场景请填写“公众号 AppID”。它不是商户号、不是微信支付公钥 ID，也不要填开放平台网站应用 AppID。',
        appSecret: 'App Secret',
        appSecretPlaceholder: '********',
        appSecretHint: '填写与上方公众号 AppID 对应的 App Secret，用于网页授权后端换取 openid（请保密）。',
        appSecretConfiguredPlaceholder: '********',
        appSecretConfiguredHint: '密钥已配置，留空以保留当前值。',
        mode: 'Mode',
        modePlaceholder: '例如 open',
        modeHint: '后端定义的模式字符串。保持默认 `open` 即可，除非你明确知道当前部署改了微信 OAuth provider 模式。',
        scopes: 'Scopes',
        scopesPlaceholder: 'snsapi_base',
        scopesHint:
          '使用空格分隔多个 scope。当前项目微信内支付推荐填写 `snsapi_base`；如果确实需要用户资料可改为 `snsapi_userinfo`。不要填 `snsapi_login`，它是网站扫码登录常见 scope，不适用于这里这条公众号网页授权链路。',
        redirectUrl: '后端回调地址（Redirect URL）',
        redirectUrlPlaceholder: 'https://your-domain.com/api/v1/auth/oauth/wechat/callback',
        redirectUrlHint: '这里必须填写完整的 http(s) 后端回调 URL。微信公众平台侧配置的是“网页授权域名”，只填域名本身，不是这个完整地址，也不是 JS 安全域名。',
        frontendRedirectUrl: '前端回调路径',
        frontendRedirectUrlPlaceholder: '/auth/wechat/callback',
        frontendRedirectUrlHint: '后端完成微信回调后会跳转到此前端路径，用于恢复支付/登录流程；通常保持默认即可。',
        quickSetCopy: '使用当前站点生成并复制',
        redirectUrlSetAndCopied: '已使用当前站点生成回调地址并复制到剪贴板'
      },
      oidc: {
      ...
      payment: {
        ...
        field_publicKeyId: '微信支付公钥 ID',
        field_certSerial: '商户证书序列号',
        field_secretKey: '密钥',
        ...
        providerConfig: '凭证配置',
        wxpayGuideTitle: '微信支付官方凭证说明',
        wxpayGuideDescription: '这里填的是“微信支付商户平台”参数，只负责真正发起支付；它不包含公众号网页授权配置。',
        wxpayGuideCompatibility: '如果还要支持“微信内直接拉起支付”，除了这里的商户参数，还必须在上面的 WeChat Connect 区配置公众号网页授权参数来获取 openid。',
        wxpayGuideNoteAppId: '`appId` 建议填写与商户号绑定的公众号 AppID。当前项目微信内支付场景不要把这里和公众号后台里的“网页授权域名 / JS 安全域名 / 消息推送”混为一谈。',
        wxpayGuideNoteApi: '`publicKey` / `publicKeyId` / `apiV3Key` / `certSerial` 都来自微信支付商户平台的 API 安全页，不是公众号后台字段。',
        alipayGuideTitle: '支付宝官方凭证说明',
        alipayGuideDescription: '这里填的是支付宝开放平台应用参数。通常需要先在开放平台创建应用，再在“开发设置/接口加签方式”里拿到 AppID、应用私钥和支付宝公钥。',
        alipayGuideNoteApp: '`appId` 来自支付宝开放平台应用详情页，不是 PID。',
        alipayGuideNoteKeys: '`privateKey` 是你的应用私钥，`publicKey` 填支付宝公钥；可在开放平台的密钥工具和应用配置页生成/校验。',
        alipayGuideNoteIpWhitelist: '如果调用时报 `40006 当前调用IP不在可信名单中`，需要到支付宝开放平台把当前服务器实际出口 IP 加入可信白名单。',
        linkWechatPublicPlatform: '微信公众平台',
        linkWechatMerchantPlatform: '微信支付商户平台',
        linkWechatJsapiGuide: '微信支付接入说明',
        linkWechatParamsGuide: '微信支付参数说明',
        linkAlipayOpenPlatform: '支付宝开放平台',
        linkAlipayDevTools: '支付宝密钥工具',
        wxpayFieldHintAppId: '填写已绑定当前商户号的 AppID。微信内支付推荐使用公众号 AppID。',
        wxpayFieldHintMchId: '微信支付商户号，来自微信支付商户平台。',
        wxpayFieldHintApiV3Key: '来自微信支付商户平台「API 安全」中的 APIv3 密钥，长度必须为 32 字节。',
        wxpayFieldHintPublicKey: '填写微信支付公钥，不是你自己的应用公钥。',
        wxpayFieldHintPublicKeyId: '与上方微信支付公钥配套的 Key ID，来自商户平台 API 安全页。',
        wxpayFieldHintCertSerial: '填写商户 API 证书序列号，对应你用于签名请求的商户证书。',
        alipayFieldHintAppId: '填写支付宝开放平台应用的 AppID。',
        alipayFieldHintPrivateKey: '填写你自己的应用私钥，通常在开放平台密钥工具中生成。',
        alipayFieldHintPublicKey: '填写支付宝公钥，用于验签支付宝返回内容。',
        noProviders: '暂无服务商实例',
  ...
  payment: {
    ...
    wechat: {
      callbackRedirecting: '正在返回支付页面...',
      authRedirecting: '正在跳转微信授权...',
      authRetrying: '正在重新拉起微信授权...',
      authResuming: '微信授权完成，正在继续支付...',
      authResumeFailed: '微信授权已完成，但支付仍然要求重新授权，请重新发起支付。',
      authMissingUrl: '缺少微信授权地址，请稍后重试',
      authExpired: '微信授权恢复信息已失效，请重新发起支付',
      authStateMismatch: '微信授权状态与已保存的支付上下文不匹配，请重新发起支付',
      authRetryExceeded: '微信授权重试次数过多，请重新发起支付',
      callbackMissingOpenId: '微信未返回可用的 OpenID，请重新发起支付',
      resumePlanMissing: '未找到待恢复的订阅套餐，请重新选择后支付',
      jsapiInvoking: '正在拉起微信支付...',
      jsapiWaiting: '请在微信内完成支付',
      jsapiPendingHint: '微信支付已拉起，完成支付后页面会自动刷新结果',
      jsapiProcessing: '支付已提交，正在等待微信确认结果...',
      jsapiCancelled: '您已取消微信支付，可重新发起或取消订单',
      jsapiUnavailable: '当前环境无法拉起微信支付，请在微信内重试',
      jsapiInvalidPayload: '微信支付参数不完整，请重新发起支付',
      jsapiFailed: '微信支付拉起失败：{reason}',
    },
    orders: {
      title: '我的订单',
      empty: '暂无订单',
