export default {
  ...
  admin: {
    ...
    settings: {
      ...
      wechat: {
        title: '微信 Connect 登录',
        description: '配置微信 Connect OAuth，用于登录/注册页入口',
        enable: '启用微信登录',
        enableHint: '为登录/注册页面准备微信登录入口',
        paymentUsageTitle: '微信内支付也会使用这一组参数',
        paymentUsageDesc: '当用户在微信内选择 `wxpay` / `wxpay_direct` 时，后端会先走公众号网页授权，用这里的 AppID / AppSecret 获取 openid，然后再拉起微信支付。',
        paymentCompatibilityHint: '如果这里没有配置：支付宝和站外支付不受影响，但微信内的 `wxpay` / `wxpay_direct` 将无法自动获取 openid，创建订单会直接报配置错误。',
        ...
        appIdHint: '微信内支付场景请填写“公众号 AppID”；它不是商户号，也不是微信支付公钥 ID。',
        ...
        modeHint: '后端定义的模式字符串，需与后端实现和微信侧应用配置保持一致。',
        scopes: 'Scopes',
        scopesPlaceholder: 'snsapi_login',
        scopesHint:
          '使用空格分隔多个 scope。网站扫码登录常见为 `snsapi_login`，应用内流程常见为 `snsapi_base` / `snsapi_userinfo`。',
        redirectUrl: '后端回调地址（Redirect URL）',
        redirectUrlPlaceholder: 'https://your-domain.com/api/v1/auth/oauth/wechat/callback',
        redirectUrlHint: '必须与微信侧登记的回调地址一致，并保持为完整的 http(s) URL。',
        frontendRedirectUrl: '前端回调路径',
        frontendRedirectUrlPlaceholder: '/auth/wechat/callback',
        frontendRedirectUrlHint: '后端完成回调后跳转到此前端路径，需与后续微信回调页面保持一致。',
        quickSetCopy: '使用当前站点生成并复制',
        redirectUrlSetAndCopied: '已使用当前站点生成回调地址并复制到剪贴板'
      },
      oidc: {
      ...
      payment: {
        ...
        wxpayGuideDescription: '这里填的是微信支付商户平台参数；如果还要支持“微信内直接拉起支付”，通常还需要上面的 WeChat Connect/公众号 OAuth 配置来先拿 openid。',
        wxpayGuideCompatibility: '不配置上面的 WeChat Connect 时，微信外部浏览器仍可使用常规微信支付链路；但微信内的 `wxpay` / `wxpay_direct` 无法自动补 openid。',
        wxpayGuideNoteAppId: '`appId` 通常填写与商户号绑定的公众号 AppID / 小程序 AppID。当前项目的微信内支付场景，推荐使用已绑定商户号的公众号 AppID。',
