export default {
  ...
  admin: {
    ...
    settings: {
      ...
      useKeyTemplate: {
        title: '自定义使用密钥模板',
        description: '为各客户端自定义"使用密钥"弹窗中的配置模板。留空则使用默认模板。',
        client: '客户端',
        system: '系统 / Shell',
        file: '文件',
        editorLabel: '模板内容',
        placeholderHint: '支持变量：${baseUrl}（API 端点）、${apiKey}（密钥）。留空则使用上方显示的默认模板。',
        save: '暂存当前模板',
        resetToDefault: '填入默认值',
        saved: '模板已暂存',
        savedList: '已自定义的模板',
        saveHint: '完成修改后仍需要点击页面最下方"保存设置"按钮应用当前修改。',
      },
      payment: {
        title: '支付设置',
