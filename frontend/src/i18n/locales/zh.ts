export default {
  ...
  auth: {
    ...
    wechat: {
      ...
      qrCodeAlt: '微信公众号二维码',
      noQrCode: '二维码未配置',
      scanTip: '使用微信扫描上方二维码即可登录',
      scanTipOfficial: '使用微信扫描上方二维码，关注公众号后发送【验证码】获取验证码',
      scanAutoTip: '使用微信扫描上方二维码，扫码即可自动登录',
      scanWaiting: '等待扫码...',
      scanConfirmed: '扫码成功，正在登录...',
      scanExpired: '二维码已过期',
      scanRefresh: '刷新二维码',
      scanError: '扫码登录失败',
      codeLabel: '验证码',
    ...
    emailBind: {
      ...
      descriptionRequired: '为保障账户安全，首次使用需绑定邮箱并设置密码。',
  ...
  redeem: {
    ...
    wechatRecharge: '余额充值（微信支付）',
    wechatRefunded: '充值已退款（微信支付）',
    wechatPay: '微信支付',
  ...
  admin: {
    ...
    settings: {
      ...
      wechat: {
        ...
        accountType: '公众号类型',
        accountTypeSubscription: '订阅号',
        accountTypeUnverifiedOfficial: '未认证公众号',
        accountTypeHint: '选择您使用的微信公众号类型，不同类型的登录提示文案会有所不同',
  ...
  subscriptionPlan: {
    ...
    upgradeKeyGuideTitle: '升级已完成',
    upgradeKeyGuideMessage: '检测到本次为套餐升级。为避免调用失败，建议现在前往 API 密钥页面，将正在使用的密钥分组更新为新套餐。',
    goToKeys: '去 API 密钥页面',
    later: '稍后处理',
  ...
  onboarding: {
    ...
    user: {
      welcome: {
        ...
        descriptionStandard: '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 Code80 AI 服务平台。</p><p style="margin-bottom: 12px;"><b>快速开始三步走：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>创建 API 密钥</li><li>了解订阅和余额</li><li>开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">跟着指引走，马上就能用 →</p></div>',
        nextBtn: '开始',
        prevBtn: '跳过'
      },
      keyManage: {
      ...
      subscriptions: {
        title: '订阅与余额',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">密钥创建好了，接下来了解下如何获取使用额度。</p><div style="padding: 8px 12px; background: #eff6ff; border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>关于余额：</b><ul style="margin: 8px 0 0 16px;"><li>调用 API 会消耗余额</li><li>余额不足时请求将被拒绝</li><li>可以在"我的订阅"页面查看当前余额和用量</li></ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>提示：</b>先了解余额获取方式，再开始使用服务</p></div>'
      },
      complete: {
        title: '引导完成',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;">密钥已准备就绪，还需要获取余额才能开始使用。</p><div style="padding: 8px 12px; background: #eff6ff; border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 13px; margin-bottom: 12px;"><b>获取余额的方式：</b><ul style="margin: 8px 0 0 16px;">__METHODS__</ul></div><p style="padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>准备好了？</b><br/>获取余额后，将密钥配置到支持 OpenAI 接口的客户端即可开始使用！</p></div>',
        methodPurchase: '购买订阅套餐',
        methodRecharge: '在线充值',
        methodRedeem: '兑换码兑换'
      }
    }
