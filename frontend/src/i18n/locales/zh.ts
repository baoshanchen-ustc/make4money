export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      cpaImport: '从 CPA 导入',
      cpaImportTitle: '从 CPA 导入',
      cpaImportModeRemote: '连接 CPA',
      cpaImportModeFile: '上传文件',
      cpaImportHint: '上传从 CLIProxyAPI 下载的认证 JSON 文件，导入为 sub2api 账号。',
      cpaImportRemoteHint: '连接 CLIProxyAPI 管理地址进行预览和导入。后端只会展示并导入 status=active 的账号。',
      cpaImportWarning: '目前支持 Claude / Codex / Gemini / Antigravity 认证文件。上传文件模式无法校验远端 status，请仅用于你已确认正常的 auth 文件。已有同源账号会更新凭据；新账号会按下方默认选项创建。',
      cpaImportRemoteWarning: '只会导入状态正常的 CPA 认证文件。非正常状态和不支持的 provider 会在后端预览阶段直接过滤。',
      cpaRemoteBaseUrl: 'CPA 服务地址',
      cpaRemoteBaseUrlPlaceholder: '例如：https://example.com',
      cpaRemoteManagementKey: 'Management Key',
      cpaRemoteMissingFields: '请填写 CPA 服务地址和 Management Key',
      cpaRemotePreviewSummaryTitle: '预览摘要',
      cpaRemotePreviewSummary: '共发现 {total} 个认证文件，可导入 {importable} 个，已过滤非正常状态 {skipped_non_normal} 个，不支持类型 {skipped_unsupported} 个。',
      cpaRemoteStatusFilterNote: '下方只展示 status=active 的可导入账号。',
      cpaRemoteExistingAccounts: '将自动更新的已有账号',
      cpaRemoteNewAccounts: '新账号（可选择）',
      cpaRemoteNoImportableAccounts: '当前没有可导入的正常状态 CPA 账号。',
      cpaImportFile: 'CPA 认证文件',
      cpaImportSelectFile: '请选择 CPA 认证文件',
      cpaImportPreview: '解析预览',
      cpaImportPreviewing: '解析中...',
      cpaImportPreviewFailed: 'CPA 文件预览失败',
      cpaImportButton: '开始导入',
      cpaImporting: '导入中...',
      cpaImportFailed: 'CPA 导入失败',
      cpaImportResult: '导入结果',
      cpaImportResultSummary: '创建 {created}，更新 {updated}，失败 {failed}',
      cpaImportErrors: '失败详情',
      cpaImportSuccess: 'CPA 导入完成：创建 {created}，更新 {updated}',
      cpaImportCompletedWithErrors: 'CPA 导入完成但有错误：失败 {failed}',
      cpaImportExistingTitle: '检测到已有导入账号',
      cpaImportExistingDesc: '将更新现有账号 “{name}” 的凭据和来源元数据，不覆盖当前代理、分组和并发配置。',
      cpaImportConcurrency: '默认并发数',
      cpaUseDefaultGroupBind: '新账号自动绑定平台默认分组（推荐）',
      cpaManualGroupsHint: '关闭默认分组后，可手动选择要绑定的分组。仅对新创建账号生效。',
      cpaManualGroupsMultiPlatformHint: '只有当选中的新 CPA 账号都属于同一平台时，才能手动选分组。混合平台导入请保持默认分组绑定开启。',
