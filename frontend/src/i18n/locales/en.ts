  ...
  admin: {
    ...
    settings: {
      ...
      wechat: {
        ...
        authDomainCallbackHint: 'The "Backend Redirect URL" here should be a full callback URL such as `https://crs.qazwc.com/api/v1/auth/oauth/wechat/callback`, while the Official Account backend only stores the bare domain `crs.qazwc.com`.',
        mpVerifyTitle: 'Domain verification file note',
        mpVerifyDesc: 'If WeChat Official Account asks you to upload an `MP_verify_xxx.txt` file, WeChat validates the final public URL. That verification is separate from the callback URL configured here.',
        mpVerifyProxyHint: 'If the site is behind Nginx, Caddy, aaPanel, or a CDN, place the file at the final public site root or return `/MP_verify_xxx.txt` directly at the reverse proxy layer. Do not rely on an app container path that is still hidden behind a full-site reverse proxy.',
        mpVerifyAccessHint: 'After configuration, make sure `https://your-domain.com/MP_verify_xxx.txt` is directly accessible and returns the plain text file content.',
        linkPublicPlatform: 'Open Official Account Platform',
        linkMerchantPlatform: 'Open WeChat Pay Merchant Platform',
        linkWebAuthGuide: 'View webpage OAuth guide',
        linkJsapiGuide: 'View JSAPI / Native guide',
        publicSettingsHint:
        ...
        appIdHint: 'For the current in-WeChat payment flow, use the Official Account AppID. It is not the merchant ID, not the WeChat Pay public key ID, and should not be an Open Platform website-login AppID.',
        ...
        redirectUrlHint:
          'This field must be a full absolute http(s) backend callback URL. In the Official Account backend you configure the "Webpage Authorization Domain" only, not this full URL and not the JS Security Domain.',
        frontendRedirectUrl: 'Frontend Callback Path',
        frontendRedirectUrlPlaceholder: '/auth/wechat/callback',
        frontendRedirectUrlHint:
          'Frontend route used after the backend callback completes to resume payment/login flow. Keeping the default is usually sufficient.',
        quickSetCopy: 'Generate & Copy (current site)',
        redirectUrlSetAndCopied: 'Redirect URL generated and copied to clipboard'
      },
      oidc: {
      ...
      payment: {
        ...
        wxpayGuideTitle: 'WeChat Pay credential guide',
        wxpayGuideDescription: 'These fields come from the WeChat Pay merchant platform and are only used for the actual payment request. They do not configure Official Account webpage OAuth.',
        wxpayGuideCompatibility: 'If you also want direct payment inside WeChat, you still need the WeChat Connect section above to provide Official Account webpage OAuth and resolve openid first.',
        wxpayGuideNoteAppId: '`appId` should usually be the Official Account AppID bound to the merchant. In the current in-WeChat payment flow, do not confuse this section with Official Account settings such as Webpage Authorization Domain, JS Security Domain, or message push configuration.',
        wxpayGuideNoteApi: '`publicKey` / `publicKeyId` / `apiV3Key` / `certSerial` all come from the WeChat Pay merchant platform API Security page, not from the Official Account backend.',
        alipayGuideTitle: 'Alipay credential guide',
        alipayGuideDescription: 'These fields come from an app in Alipay Open Platform. Normally you create an app first, then retrieve AppID, your app private key, and Alipay public key from the development/signing settings.',
        alipayGuideNoteApp: '`appId` comes from the Alipay Open Platform app details page, not from PID.',
        alipayGuideNoteKeys: '`privateKey` is your application private key, while `publicKey` should be the Alipay public key used for signature verification.',
        alipayGuideNoteIpWhitelist: 'If requests fail with `40006 current IP is not in the trusted list`, add the server egress IP to the trusted IP whitelist in Alipay Open Platform.',
        linkWechatPublicPlatform: 'Official Account Platform',
        linkWechatMerchantPlatform: 'WeChat Pay Merchant Platform',
        linkWechatJsapiGuide: 'WeChat Pay integration guide',
        linkWechatParamsGuide: 'WeChat Pay parameter guide',
        linkAlipayOpenPlatform: 'Alipay Open Platform',
        linkAlipayDevTools: 'Alipay key tools',
        wxpayFieldHintAppId: 'Use the AppID bound to the current merchant. For in-WeChat payment, an Official Account AppID is recommended.',
        wxpayFieldHintMchId: 'WeChat Pay merchant ID from the merchant platform.',
        wxpayFieldHintApiV3Key: 'APIv3 key from the WeChat Pay merchant platform API Security page. It must be 32 bytes long.',
        wxpayFieldHintPublicKey: 'Use the WeChat Pay public key, not your own application public key.',
        wxpayFieldHintPublicKeyId: 'The Key ID paired with the WeChat Pay public key above, from the merchant platform API Security page.',
        wxpayFieldHintCertSerial: 'Merchant API certificate serial number for the certificate used to sign requests.',
        alipayFieldHintAppId: 'Use the AppID from your Alipay Open Platform application.',
        alipayFieldHintPrivateKey: 'Use your application private key, usually generated by Alipay key tools.',
        alipayFieldHintPublicKey: 'Use the Alipay public key for signature verification.',
