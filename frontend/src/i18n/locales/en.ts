  ...
  admin: {
    ...
    settings: {
      ...
      wechat: {
        ...
        paymentUsageTitle: 'This credential set is also used for in-WeChat payment',
        paymentUsageDesc: 'When a user selects `wxpay` / `wxpay_direct` inside WeChat, the backend first performs Official Account webpage OAuth with this AppID / AppSecret to obtain an openid, then starts WeChat Pay.',
        paymentCompatibilityHint: 'If this section is not configured, Alipay and out-of-WeChat payments still work, but in-WeChat `wxpay` / `wxpay_direct` cannot auto-resolve openid and create-order will fail immediately.',
        ...
        appIdHint: 'For in-WeChat payments, use the Official Account AppID. It is not the merchant ID or WeChat Pay public key ID.',
      ...
      payment: {
        ...
        wxpayGuideDescription: 'These fields come from the WeChat Pay merchant platform. If you also want to support direct payment inside WeChat, you usually need the WeChat Connect / Official Account OAuth settings above to resolve openid first.',
        wxpayGuideCompatibility: 'Without the WeChat Connect section above, standard WeChat Pay flows outside WeChat can still work, but in-WeChat `wxpay` / `wxpay_direct` cannot auto-resolve openid.',
        wxpayGuideNoteAppId: '`appId` is usually the Official Account AppID or Mini Program AppID bound to the merchant. For the current in-WeChat payment flow, an Official Account AppID is the recommended choice.',
