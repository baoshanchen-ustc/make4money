...
sub2api 推荐使用 `ANTHROPIC_AUTH_TOKEN`（上行 `Authorization: Bearer ...`）而非 `ANTHROPIC_API_KEY`，并配合 `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`（抑制 Claude Code 内置 metrics / autoupdater / error sink 等 nonessential traffic）与 `CLAUDE_CODE_ATTRIBUTION_HEADER=0`（关闭 first-party billing attribution header）。

> 详细说明、BigQuery metrics 边界、响应头 gateway detection 范围与回滚开关详见 [`hook/docs/sub2api-claude-code-usage-guide.md`](hook/docs/sub2api-claude-code-usage-guide.md)。
