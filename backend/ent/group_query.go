...
func (_q *GroupQuery) QuerySubscriptionOrders() *SubscriptionOrderQuery {
...
func (_q *GroupQuery) WithSubscriptionOrders(opts ...func(*SubscriptionOrderQuery)) *GroupQuery {
	query := (&SubscriptionOrderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withSubscriptionOrders; query != nil {
		if err := _q.loadSubscriptionOrders(ctx, query, nodes,
			func(n *Group) { n.Edges.SubscriptionOrders = []*SubscriptionOrder{} },
			func(n *Group, e *SubscriptionOrder) {
...
func (_q *GroupQuery) loadSubscriptionOrders(ctx context.Context, query *SubscriptionOrderQuery, nodes []*Group, init func(*Group), assign func(*Group, *SubscriptionOrder)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionorder.FieldGroupID)
	}
	query.Where(predicate.SubscriptionOrder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.SubscriptionOrdersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
