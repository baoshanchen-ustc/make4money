...
func (_q *UserQuery) QueryCheckins() *UserCheckInQuery {
...
func (_q *UserQuery) WithCheckins(opts ...func(*UserCheckInQuery)) *UserQuery {
	query := (&UserCheckInClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withCheckins; query != nil {
		if err := _q.loadCheckins(ctx, query, nodes,
			func(n *User) { n.Edges.Checkins = []*UserCheckIn{} },
			func(n *User, e *UserCheckIn) { n.Edges.Checkins = append(n.Edges.Checkins, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadCheckins(ctx context.Context, query *UserCheckInQuery, nodes []*User, init func(*User), assign func(*User, *UserCheckIn)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(usercheckin.FieldUserID)
	}
