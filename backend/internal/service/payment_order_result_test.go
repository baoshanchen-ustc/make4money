...
func (s paymentOrderResultSettingRepoStub) Get(context.Context, string) (*Setting, error) {
	return nil, ErrSettingNotFound
}

func (s paymentOrderResultSettingRepoStub) GetValue(_ context.Context, key string) (string, error) {
...
func (s paymentOrderResultSettingRepoStub) Set(context.Context, string, string) error { return nil }

func (s paymentOrderResultSettingRepoStub) GetMultiple(_ context.Context, keys []string) (map[string]string, error) {
...
func (s paymentOrderResultSettingRepoStub) SetMultiple(context.Context, map[string]string) error {
	return nil
}

func (s paymentOrderResultSettingRepoStub) GetAll(context.Context) (map[string]string, error) {
	return s.values, nil
}

func (s paymentOrderResultSettingRepoStub) Delete(context.Context, string) error { return nil }
...
func TestMaybeBuildWeChatOAuthRequiredResponse(t *testing.T) {
	...
	svc := &PaymentService{
		configService: NewPaymentConfigService(nil, paymentOrderResultSettingRepoStub{values: map[string]string{
			SettingKeyWeChatLoginMPEnabled:   "true",
			SettingKeyWeChatLoginMPAppID:     "wx123456",
			SettingKeyWeChatLoginMPAppSecret: "wechat-secret",
		}}, nil),
	}
...
func TestMaybeBuildWeChatOAuthRequiredResponseRequiresMPConfigInWeChat(t *testing.T) {
	...
	svc := &PaymentService{
		configService: NewPaymentConfigService(nil, paymentOrderResultSettingRepoStub{values: map[string]string{
			SettingKeyWeChatLoginMPEnabled: "false",
		}}, nil),
	}
...
func TestMaybeBuildWeChatOAuthRequiredResponseForSelection_SkipsEasyPayProvider(t *testing.T) {
	t.Parallel()
	...
	if resp != nil {
		t.Fatalf("expected nil response for easypay-backed wxpay, got %+v", resp)
	}
}

func TestClassifyCreatePaymentErrorWechatH5NoAuth(t *testing.T) {
	t.Parallel()

	err := classifyCreatePaymentError(
		CreateOrderRequest{PaymentType: payment.TypeWxpay, IsMobile: true},
		payment.TypeWxpay,
		errors.New(`wxpay h5 payments are not authorized for this merchant: wxpay h5 prepay: error http response:[StatusCode: 403 Code: "NO_AUTH"]`),
	)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	...
	if got := appErr.Metadata["action"]; got != "open_in_wechat_or_scan_qr" {
		t.Fatalf("metadata.action = %q, want %q", got, "open_in_wechat_or_scan_qr")
	}
}
