...
func TestSettingHandler_GetPublicSettings_ExposesFeatureSwitches(t *testing.T) {
	gin.SetMode(gin.TestMode)

	repo := &settingHandlerPublicRepoStub{
		values: map[string]string{
			service.SettingKeyAvailableChannelsEnabled: "true",
			service.SettingKeyServiceQuotaEnabled:      "true",
		},
	}
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{}), "test-version")
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.True(t, resp.Data.AvailableChannelsEnabled)
	require.True(t, resp.Data.ServiceQuotaEnabled)
}
