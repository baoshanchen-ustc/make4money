...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, planType string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	if planType != "" {
		q = q.Where(func(s *entsql.Selector) {
			s.Where(sqljson.ValueEQ(dbaccount.FieldCredentials, planType, sqljson.Path("plan_type")))
		})
	}
