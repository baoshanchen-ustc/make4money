...
func TestCreatePaymentWithOpenIDPrefersWechatMpAppIDForJSAPI(t *testing.T) {
	origJSAPIPrepay := wxpayJSAPIPrepayWithRequestPayment
	t.Cleanup(func() {
		wxpayJSAPIPrepayWithRequestPayment = origJSAPIPrepay
	})
	...
	wxpayJSAPIPrepayWithRequestPayment = func(ctx context.Context, svc jsapi.JsapiApiService, req jsapi.PrepayRequest) (*jsapi.PrepayWithRequestPaymentResponse, *core.APIResult, error) {
		if got := wxSV(req.Appid); got != "wx-mp-login" {
			t.Fatalf("jsapi appid = %q, want %q", got, "wx-mp-login")
		}
		return &jsapi.PrepayWithRequestPaymentResponse{
			Appid: core.String("wx-mp-login"),
		}, nil, nil
	}
	...
	if resp.JSAPI.AppID != "wx-mp-login" {
		t.Fatalf("resp.JSAPI.AppID = %q, want %q", resp.JSAPI.AppID, "wx-mp-login")
	}
}

func TestAppendWxpayRedirectURL(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:  "empty return url keeps h5 url unchanged",
			h5URL: "https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb?prepay_id=wx123&package=abc",
			req: payment.CreatePaymentRequest{
				OrderID:     "sub2_46",
				PaymentType: payment.TypeWxpay,
			},
			want: "https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb?prepay_id=wx123&package=abc",
		},
		{
			name:  "rewrites source page to payment result redirect",
			h5URL: "https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb?prepay_id=wx123&package=abc",
			req: payment.CreatePaymentRequest{
				OrderID:     "sub2_46",
				PaymentType: payment.TypeWxpay,
				ReturnURL:   "https://merchant.example/purchase?plan=pro&coupon=summer",
			},
			want: "https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb?prepay_id=wx123&package=abc&redirect_url=https%3A%2F%2Fmerchant.example%2Fpayment%2Fresult%3Fout_trade_no%3Dsub2_46%26payment_type%3Dwxpay",
		},
		{
			name:  "supports h5 url without query",
			h5URL: "https://wx.tenpay.com/pay",
			req: payment.CreatePaymentRequest{
				OrderID:   "sub2_99",
				ReturnURL: "https://merchant.example/purchase",
			},
			want: "https://wx.tenpay.com/pay?redirect_url=https%3A%2F%2Fmerchant.example%2Fpayment%2Fresult%3Fout_trade_no%3Dsub2_99",
		},
		{
			name:  "invalid return url errors",
			h5URL: "https://wx.tenpay.com/cgi-bin/mmpayweb-bin/checkmweb?prepay_id=wx123&package=abc",
			req: payment.CreatePaymentRequest{
				OrderID:   "sub2_46",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got, err := appendWxpayRedirectURL(tt.h5URL, tt.req)
			if tt.wantErr != "" {
				if err == nil {
					t.Fatal("expected error, got nil")
				}
			...
			if got != tt.want {
				t.Fatalf("appendWxpayRedirectURL() = %q, want %q", got, tt.want)
			}
		})
	}
}

func TestBuildWxpayResultURL(t *testing.T) {
	t.Parallel()

	got, err := buildWxpayResultURL("https://merchant.example/purchase?plan=pro#section", payment.CreatePaymentRequest{
		OrderID:     "sub2_123",
		PaymentType: payment.TypeWxpay,
	})
	if err != nil {
	...
	if parsed.Scheme != "https" || parsed.Host != "merchant.example" {
		t.Fatalf("result origin = %s://%s, want https://merchant.example", parsed.Scheme, parsed.Host)
	}
	if parsed.Path != wxpayResultPath {
		t.Fatalf("result path = %q, want %q", parsed.Path, wxpayResultPath)
	}
	if parsed.RawFragment != "" {
		t.Fatalf("result fragment = %q, want empty", parsed.RawFragment)
	}
	if gotOutTradeNo := parsed.Query().Get("out_trade_no"); gotOutTradeNo != "sub2_123" {
		t.Fatalf("out_trade_no = %q, want %q", gotOutTradeNo, "sub2_123")
	}
	if gotPaymentType := parsed.Query().Get("payment_type"); gotPaymentType != payment.TypeWxpay {
		t.Fatalf("payment_type = %q, want %q", gotPaymentType, payment.TypeWxpay)
	}
}

func TestCreatePaymentH5NoAuthReturnsExplicitError(t *testing.T) {
	origH5Prepay := wxpayH5Prepay
	t.Cleanup(func() {
		wxpayH5Prepay = origH5Prepay
	})
	...
	wxpayH5Prepay = func(ctx context.Context, svc h5.H5ApiService, req h5.PrepayRequest) (*h5.PrepayResponse, *core.APIResult, error) {
		return nil, nil, fmt.Errorf(`error http response:[StatusCode: 403 Code: "NO_AUTH"]`)
	}
	...
	if !strings.Contains(err.Error(), "not authorized") {
		t.Fatalf("error = %q, want not authorized", err.Error())
	}
}
