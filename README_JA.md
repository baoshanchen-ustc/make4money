...
sub2api の推奨テンプレートでは `ANTHROPIC_API_KEY` ではなく `ANTHROPIC_AUTH_TOKEN`（`Authorization: Bearer ...` として送信）を使用し、`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`（Claude Code 内蔵の metrics / autoupdater / error sink などの nonessential traffic を抑制）と `CLAUDE_CODE_ATTRIBUTION_HEADER=0`（first-party billing attribution header を無効化）を併用します。

> 詳細な根拠、BigQuery metrics の境界、レスポンスヘッダの gateway detection 範囲、ロールバック方法は [`hook/docs/sub2api-claude-code-usage-guide.md`](hook/docs/sub2api-claude-code-usage-guide.md)（中国語）を参照してください。
